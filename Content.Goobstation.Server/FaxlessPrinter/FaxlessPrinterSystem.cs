...
using Content.Goobstation.Shared.FaxlessPrinter.Components;
...
namespace Content.Goobstation.Server.FaxlessPrinter;
...
public sealed class FaxlessPrinterSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        // Hooks
        SubscribeLocalEvent<FaxlessPrinterComponent, ComponentInit>(OnComponentInit);
        SubscribeLocalEvent<FaxlessPrinterComponent, MapInitEvent>(OnMapInit);
        SubscribeLocalEvent<FaxlessPrinterComponent, ComponentRemove>(OnComponentRemove);

        SubscribeLocalEvent<FaxlessPrinterComponent, EntInsertedIntoContainerMessage>(OnItemSlotChanged);
        SubscribeLocalEvent<FaxlessPrinterComponent, EntRemovedFromContainerMessage>(OnItemSlotChanged);
        SubscribeLocalEvent<FaxlessPrinterComponent, PowerChangedEvent>(OnPowerChanged);
        SubscribeLocalEvent<FaxlessPrinterComponent, DeviceNetworkPacketEvent>(OnPacketReceived);

        // Interaction
        SubscribeLocalEvent<FaxlessPrinterComponent, InteractUsingEvent>(OnInteractUsing);
        SubscribeLocalEvent<FaxlessPrinterComponent, GotEmaggedEvent>(OnEmagged);

        // UI
        SubscribeLocalEvent<FaxlessPrinterComponent, AfterActivatableUIOpenEvent>(OnToggleInterface);
        SubscribeLocalEvent<FaxlessPrinterComponent, FaxlessPrinterFileMessage>(OnFileButtonPressed);
        SubscribeLocalEvent<FaxlessPrinterComponent, FaxlessPrinterCopyMessage>(OnCopyButtonPressed);
        SubscribeLocalEvent<FaxlessPrinterComponent, FaxlessPrinterSendMessage>(OnSendButtonPressed);
        SubscribeLocalEvent<FaxlessPrinterComponent, FaxlessPrinterRefreshMessage>(OnRefreshButtonPressed);
        SubscribeLocalEvent<FaxlessPrinterComponent, FaxlessPrinterDestinationMessage>(OnDestinationSelected);
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<FaxlessPrinterComponent, ApcPowerReceiverComponent>();
    ...
    private void ProcessPrintingAnimation(EntityUid uid, float frameTime, FaxlessPrinterComponent comp)
    {
    ...
    private void ProcessInsertingAnimation(EntityUid uid, float frameTime, FaxlessPrinterComponent comp)
    {
    ...
    private void ProcessSendingTimeout(EntityUid uid, float frameTime, FaxlessPrinterComponent comp)
    {
    ...
    private void OnComponentInit(EntityUid uid, FaxlessPrinterComponent component, ComponentInit args)
    {
    ...
    private void OnComponentRemove(EntityUid uid, FaxlessPrinterComponent component, ComponentRemove args)
    {
        _itemSlotsSystem.RemoveItemSlot(uid, component.PaperSlot);
    }

    private void OnMapInit(EntityUid uid, FaxlessPrinterComponent component, MapInitEvent args)
    {
        // Load all faxes on map in cache each other to prevent taking same name by user created fax
        Refresh(uid, component);
    }

    private void OnItemSlotChanged(EntityUid uid, FaxlessPrinterComponent component, ContainerModifiedMessage args)
    {
    ...
    private void OnPowerChanged(EntityUid uid, FaxlessPrinterComponent component, ref PowerChangedEvent args)
    {
    ...
    private void OnInteractUsing(EntityUid uid, FaxlessPrinterComponent component, InteractUsingEvent args)
    {
        if (args.Handled ||
    ...
    private void OnEmagged(EntityUid uid, FaxlessPrinterComponent component, ref GotEmaggedEvent args)
    {
    ...
    private void OnPacketReceived(EntityUid uid, FaxlessPrinterComponent component, DeviceNetworkPacketEvent args)
    {
        ...
        if (args.Data.TryGetValue(DeviceNetworkConstants.Command, out string? command))
        {
            switch (command)
            {
                ...
                case FaxConstants.FaxPrintCommand:
                    ...
                    var printout = new Content.Goobstation.Shared.FaxlessPrinter.Components.FaxPrintout(content, name, label, prototypeId, stampState, stampedBy, locked ?? false);
    ...
    private void OnToggleInterface(EntityUid uid, FaxlessPrinterComponent component, AfterActivatableUIOpenEvent args)
    {
        UpdateUserInterface(uid, component);
    }

    private void OnFileButtonPressed(EntityUid uid, FaxlessPrinterComponent component, FaxlessPrinterFileMessage args)
    {
    ...
    private void OnCopyButtonPressed(EntityUid uid, FaxlessPrinterComponent component, FaxlessPrinterCopyMessage args)
    {
    ...
    private void OnSendButtonPressed(EntityUid uid, FaxlessPrinterComponent component, FaxlessPrinterSendMessage args)
    {
        // Goobstation
    ...
    private void OnRefreshButtonPressed(EntityUid uid, FaxlessPrinterComponent component, FaxRefreshMessage args)
    {
        Refresh(uid, component);
    }

    private void OnDestinationSelected(EntityUid uid, FaxlessPrinterComponent component, FaxDestinationMessage args)
    {
        SetDestination(uid, args.Address, component);
    }

    private void UpdateAppearance(EntityUid uid, FaxlessPrinterComponent? component = null)
    {
    ...
    private void UpdateUserInterface(EntityUid uid, FaxlessPrinterComponent? component = null)
    {
        if (!Resolve(uid, ref component))
            return;
        ...
        var canCopy = isPaperInserted &&
                      component.SendTimeoutRemaining <= 0 &&
                      component.InsertingTimeRemaining <= 0;
        var state = new FaxlessPrinterUiState(component.FaxName, component.KnownFaxes, canSend, canCopy, isPaperInserted, component.DestinationFaxAddress);
        _userInterface.SetUiState(uid, FaxlessPrinterUiKey.Key, state);
    }

    /// <summary>
    ///     Set fax destination address not checking if he knows it exists
    /// </summary>
    public void SetDestination(EntityUid uid, string destAddress, FaxlessPrinterComponent? component = null)
    {
        if (!Resolve(uid, ref component))
            return;
    ...
    /// </summary>
    public void Refresh(EntityUid uid, FaxlessPrinterComponent? component = null)
    {
        if (!Resolve(uid, ref component))
            return;
    ...
    /// </summary>
    public void PrintFile(EntityUid uid, FaxlessPrinterComponent component, FaxFileMessage args)
    {
        ...
        var printout = new Content.Goobstation.Shared.FaxlessPrinter.Components.FaxPrintout(args.Content, name, args.Label, prototype);
    ...
    ///     which is shared by the send button.
    /// </summary>
    public void Copy(EntityUid uid, FaxlessPrinterComponent? component, FaxCopyMessage args)
    {
        if (!Resolve(uid, ref component))
            return;
        ...
        var printout = new Content.Goobstation.Shared.FaxlessPrinter.Components.FaxPrintout(paper.Content,
    ...
    /// </summary>
    public void Send(EntityUid uid, FaxlessPrinterComponent? component, FaxSendMessage args)
    {
        if (!Resolve(uid, ref component))
            return;
    ...
    public void Receive(EntityUid uid, Content.Goobstation.Shared.FaxlessPrinter.Components.FaxPrintout? printout, string? fromAddress = null, FaxlessPrinterComponent? component = null)
    {
        if (!Resolve(uid, ref component))
            return;
    ...
    private void SpawnPaperFromQueue(EntityUid uid, FaxlessPrinterComponent? component = null)
    {
